...
class _LessonScreenState extends State<LessonScreen> {
  ...
  Future<void> _loadFlashcards() async {
    ...
    final result = await ContentService.flashcards(
        widget.courseId, widget.moduleId,
        downloadedOnly: widget.downloadedOnly);
    if (!mounted) return;
    setState(() {
      _flashcards = result.items;
  ...
  void _practiceSample() {
    final result = resolveContent<Map<String, String>>(
      fetched: null,
      fallback: _getHardcodedFlashcards(widget.courseTag, widget.moduleId),
    );
    setState(() {
      _flashcards = result.items;
      _origin = result.origin;
  ...
  void _next() {
    if (_openingQuiz || _flashcards.isEmpty) return;
    if (_currentCard < _flashcards.length - 1) {
      ...
      if (_origin != ContentOrigin.substitute) {
      ...
      Navigator.pushReplacement(
        ...
        AppRouter.push(
          QuizScreen(
            ...
            practiceOnly: _origin == ContentOrigin.substitute,
          ),
        ),
      );
    }
      ...
      body: SafeArea(
        child: WebContentBounds(
            ...
            child: Column(
              ...
              children: [
                if (_origin != ContentOrigin.live)
                  Padding(
                    padding: const EdgeInsets.fromLTRB(16, 12, 16, 0),
                    child: ContentNotice(origin: _origin),
                  ),
                // ── Header ──────────────────────────────
                Padding(
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: Text(
                      _origin == ContentOrigin.substitute
                          ? 'Sample practice'
