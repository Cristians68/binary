  ...
  group('stability', () {
    ...
    test('the value is pinned, so a hashing change cannot pass silently', () {
      ...
      expect(
        ...
        //   -> 10382087631d...
        'BA-AIML-1038-2087-631D',
      );
    });
  });

  group('format', () {
    test('matches BA-CODE-XXXX-XXXX-XXXX', () {
      final id = courseCredentialId(uid: uidA, courseId: course);
      expect(
        RegExp(r'^BA-[A-Z]{2,6}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}$')
            .hasMatch(id),
        isTrue,
    ...
    test('an uncatalogued course still issues a usable credential', () {
      ...
      expect(id, startsWith('BA-$kUncataloguedCourseCode-'));
