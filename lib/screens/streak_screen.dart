...
/// On web, nested maps sometimes arrive as Map<Object, Object> — this handles it.
...
/// update() with set(merge:true) fallback — safe on iOS, Android, and Web.
...
class StreakService {
  ...
  static Future<void> recordLogin() async {
    ...
    try {
      ...
      final streak = StreakData.fromMap(_toMap(data['streak']));
      ...
      // update() with dot-notation — safe on all platforms
  ...
  static Future<void> addPoints(int points) async {
    ...
    try {
      final snapshot = await doc.get();
      final data = snapshot.data() ?? {};
      final goal = DailyGoalData.fromMap(_toMap(data['dailyGoal']));
  ...
  static Future<
    ...
    try {
      ...
      final data = snapshot.data() ?? {};
      return (
        streak: StreakData.fromMap(_toMap(data['streak'])),
        goal: DailyGoalData.fromMap(_toMap(data['dailyGoal'])),
  ...
  static Set<String> _earnedBadgeIds(Map<String, dynamic> data) {
    return _toMap(data['badges']).keys.toSet();
  }
